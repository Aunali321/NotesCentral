...
if (isset($_POST['create'])) {
    ...
    $code = $_POST['code'];
    $output = $_POST['output'];
    $sql = "INSERT INTO `practicals` (`username`, `name`, `subject`, `code`, `output`, `semester`, `pnum`) VALUES ('$username', '$name', '$subject', '$code', '$output', '$semester', '$pnum')";
    ...
    if ($result) {
        ...
        <div class="p-4 m-4 max-w-sm text-sm md:max-w-3xl mx-auto text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
            <span class="font-medium">Success!</span> Practical created successfully.
        </div>';
        header("Location: dashboard");
    } else {
        echo '
        <div class="p-4 m-4 max-w-sm text-sm md:max-w-3xl mx-auto text-green-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
            <span class="font-medium">Failed!</span> Practical couldn\'t be created.
...
<body class="bg-match">
    <div class="flex items-center mx-4 my-40 px-4 md:mx-20 md:my-28 justify-center h-screen lg:h-full">
        <div class="w-full md:m-10 bg-white border border-gray-200 rounded-lg shadow md:max-w-3xl dark:bg-gray-800 dark:border-gray-700">
            ...
            <form class="space-y-8 p-6 md:p-6 md:m-2" action="" method="POST" name="create">
                <div class="md:flex md:space-x-4">
                    <div class="mb-6 md:w-16">
                        ...
                        <input type="number" name="pnum" id="pnum" class="bg-gray-50 border text-center border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="25" required>
                    </div>
                ...
                <button type="submit" name="create" class="md:mx-auto md:w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
            </form>
        </div>
    </div>
